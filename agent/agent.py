...
class agent():
    ...
    def best_option_by_goal_winner_take_all(self):
        ...
        overall_strongest_reward = 0.0
        #print "New Timestep"
        for a_goal in self._goals:
            #print "    New goal"
            current_strongest_reward = -9999999999999999.0
            current_strongest_ply2_reward = -9999999999999999.0
            for option in self._current_options_new_pos:
                ...
                if self._ply2 == True:
                    ...
                    for option2ply in self._ply2options[option]:
                        ...
                        option_ply2_reward = a_goal.compute_total_reward(distance_ply2,velocity_current)
                        #print "                ",distance_ply2, option_ply2_reward, current_strongest_ply2_reward,current_strongest_reward,overall_strongest_reward
                        if(option_ply2_reward > current_strongest_ply2_reward):
                            current_strongest_ply2_reward = option_ply2_reward
                    if(current_strongest_ply2_reward > current_strongest_reward):
                        current_strongest_reward = current_strongest_ply2_reward
                        #print "New top current_strongest_reward",current_strongest_reward
                        best_option = option
                else:
                    distance_current = distance(a_goal._preferred_location,option)
                    option_reward = a_goal.compute_total_reward(distance_current,velocity_current)

                    if(option_reward > current_strongest_reward):
                        current_strongest_reward = option_reward
                        best_option = option


            if(abs(current_strongest_reward) > overall_strongest_reward):
                overall_strongest_reward = abs(current_strongest_reward)
                best_new_pos = best_option

        return best_new_pos,overall_strongest_reward

    def best_option_by_goal_summation(self):
        current_strongest_reward = -9999999999999999.0
        for option in self._current_options_new_pos:
            option_summed_reward = 0.0
            for a_goal in self._goals:
                distance_current = distance(a_goal._preferred_location,option)
                velocity_current = distance(option,self._prev_locations[1])/5.0
                option_reward = a_goal.compute_total_reward(distance_current,velocity_current) #compute_distance_reward(distance_current)
                option_summed_reward += option_reward
                if(option_summed_reward > current_strongest_reward):
                    current_strongest_reward = option_summed_reward
                    best_option = option

        return best_option,current_strongest_reward
    ...
    def compute_suffering(self):
        ...
        for a_goal in self._goals:
            ...
            self._suffering += a_goal.compute_total_reward(current_distance,current_velocity)
...
class goal_like():
    ...
    def compute_total_reward(self,current_distance,current_velocity):
        distance_reward = self.compute_distance_reward(current_distance)
        velocity_reward = self.compute_velocity_reward(current_velocity)
        time_reward_mod = self.compute_time_reward_modifier(current_distance)

        return ( distance_reward + velocity_reward ) * time_reward_mod

    def compute_distance_reward(self,current_distance):
        return self._base_dist_metric / pow(current_distance,self._dist_decay)

    def compute_velocity_reward(self,current_velocity):
        return self._base_velocity_metric * self.detect_velocity_satisfaction_acheivement(current_velocity)

    def compute_time_reward_modifier(self,current_distance):
    ...
    def update_world(self,count):
        for an_agent in self._agents:
            ...
            if self._choice_option == "winner_take_all":
                best_new_pos, strongest_reward = an_agent.best_option_by_goal_winner_take_all()
            elif self._choice_option == "summation":
                best_new_pos, strongest_reward = an_agent.best_option_by_goal_summation()
...
for jj in range(0,1):
    ...
    """
    Changing repulsive reward from -0.4 to -0.5 makes the difference in whether it will ever reach the upper goalstate.
    """
    ...
    print ("Look to the Future")
    world_3.run_world(iterations)
    world_3.plot_world()
    plt.show()

    print ("Higher goal satisfiability")
    ...
    print ("Less intensity AKA friendship")
